...
@section('content')
                        ...
                        <div class="row">
                            ...
                            <div class="col-xl-12">
                                <div class="card">
                                                ...
                                                @if($borrower->bdname != "")(<strong>Borrower: </strong>{{$borrower->bdname}})
                                                @endif
                                                @endforeach
                                                @endif
                                                </address>
                                                ...
                                                <b>Request State: </b> @if($requesters->state=='Not signed')
                                                ...
                                                <td class="text-end">{{ $value->request_qty}} <input type="hidden" name="count" value="{{$value->request_qty}}"></td>
                                                <td class="text-end">{{ $value->qty_given}} <input type="hidden" name="countiveng" value="{{$value->qty_given}}"></td>
                                                </tr>
                                                @endforeach
                                                @endif
                                                </tbody>
                                                ...
                                                <p class="text-end pr-4">Total items requested: <strong><span id="totalcount"></span></strong></p>
                                                <p class="text-end">Total items given: <strong><span id="totalgiven"></span></strong></p>
                                                </div>
                                                <!-- /.col -->
                                                </div>
                                                <!-- /.row -->
                                                ...
                                                <!-- /.col -->
                                                <div class="col-md-4">
                                                <p class="lead text-small">Date processed: <strong>{{$clerk->date_approved}}</strong></p>

                                                </div>
                                                <div class="col-md-4">
                                                   @if($requesters->state=='Approved' && $requestby->name==auth()->user()->name && $requesters->acknowledged==0)
                                                   <button  class="btn btn-default"  data-bs-toggle="modal" data-bs-target="#acknowledge"><i class="mdi mdi-check"></i> Acknowledge</button>
                                                    @elseif($requesters->state=='Approved' && $requesters->acknowledged==0)
                                                    <span class="badge badge-warning-lighten float-center">Not Acknowledged</span>
                                                    @elseif($requesters->state=='Approved' && $requesters->acknowledged!=0)
                                                    <span class="badge badge-success-lighten float-center">Acknowledged</span>
                                                    @endif                                               
                                                    <button  class="btn btn-default"><i class="mdi mdi-printer-check"></i> Print</button>
 ...
 <div class="modal fade" id="acknowledge" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                ...
                <form action="{{ url('inventory/request/acknowledge') }}" method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <div class="mb-3">
                                <label for="CategoryName" class="form-label"> I {{$requestby->name}} Agree that i have Received the following Items below:</label>
                                         @if(count($values)>0)
                                                @php($i=1)
                                                @foreach($values as $value)
                                                <tr>
                                                ...
                                                @endforeach
                                            @endif
                                <textarea name="approve" id="" class="form-control d-none" readonly>@php($i=1)@foreach($values as $value){{$i++}}) {{ $value->qty_given}}({{ $value->uom_name}}) {{$value->item_name}}(s) @endforeach</textarea>                                
                            </div>
